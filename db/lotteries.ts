...
export async function listLotteries(eventId: string): Promise<LotterySummary[]> {
  const rows = await getSql()<Array<{
    ...
    SELECT l.id, l.event_id, l.name, l.description, l.created_at, l.updated_at,
      ...
      (SELECT count(*)::integer FROM lottery_draws d WHERE d.lottery_id = l.id) AS winner_count,
...
export async function drawLottery(lotteryId: number, drawnBy: string) {
  return getSql().begin(async (tx) => {
    await tx`SELECT pg_advisory_xact_lock(${lotteryId})`;
    const lotteryRows = await tx<Array<{ event_id: string }>>`SELECT event_id FROM lotteries WHERE id = ${lotteryId} LIMIT 1`;
    ...
    const eligible = await tx<Array<{ device_hash: string }>>`
      SELECT cr.device_hash FROM claim_records cr
      JOIN lottery_requirements r ON r.lottery_id = ${lotteryId} AND r.event_id = cr.event_id AND r.achievement_id = cr.achievement_id
      WHERE cr.event_id = ${lottery.event_id}
        AND NOT EXISTS (SELECT 1 FROM lottery_draws d WHERE d.lottery_id = ${lotteryId} AND d.device_hash = cr.device_hash)
      GROUP BY cr.device_hash HAVING count(DISTINCT cr.achievement_id) = ${requirements[0].count}
      ORDER BY random() LIMIT 1`;
    if (!eligible[0]) return { status: "no_eligible" as const, roll };
    const drawRows = await tx<Array<{ id: number; drawn_at: string | Date }>>`
      ...
      VALUES (${lotteryId}, ${selected.id}, ${eligible[0].device_hash}, ${roll}, ${drawnBy}) RETURNING id, drawn_at`;
    ...
    return { status: "winner" as const, roll, draw: { id: drawRows[0]!.id, prizeId: selected.id,
      ...
      participantCode: participantCode(eligible[0].device_hash), drawnBy, drawnAt: toIso(drawRows[0]!.drawn_at) } };
  });
}
