...
- 所有管理员都可创建普通管理员；仅超级管理员可删除管理员并拉取 GitHub 更新
...
数据库中尚无管理员时，首次登录会使用 `.env` 中的 `ADMIN_USERNAME` 和 `ADMIN_PASSWORD` 创建唯一的超级管理员。之后这两个变量只作为首次初始化配置，任意已登录管理员都可以在“管理员面板”中创建普通管理员；删除管理员和拉取 GitHub 更新仍仅限超级管理员。每位管理员均使用自己的用户名和密码登录。
